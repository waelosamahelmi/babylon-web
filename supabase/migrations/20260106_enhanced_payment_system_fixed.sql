-- Enhanced Payment System Migration (FIXED)
-- Adds support for saved payment methods, customers, payment analytics, and receipts
-- Fixed to handle both integer and uuid order IDs

-- First, check what type the orders.id column is
DO $$
DECLARE
  order_id_type TEXT;
BEGIN
  SELECT data_type INTO order_id_type
  FROM information_schema.columns
  WHERE table_name = 'orders' AND column_name = 'id';

  RAISE NOTICE 'Orders table ID type: %', order_id_type;
END $$;
...
-- Using TEXT for order_id to be compatible with both integer and uuid
CREATE TABLE IF NOT EXISTS payment_attempts (
  id SERIAL PRIMARY KEY,
  order_id TEXT NOT NULL, -- Changed to TEXT to support both integer and uuid
...
DROP POLICY IF EXISTS saved_payment_methods_insert_own ON saved_payment_methods;
...
DROP POLICY IF EXISTS saved_payment_methods_delete_own ON saved_payment_methods;
...
DROP POLICY IF EXISTS payment_analytics_select ON payment_analytics;
